...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
     ...
     */
    List<Transaction> findByPersonIdAndTransactionTypeOrderByTransactionDateDesc(
            Long personId,
    ...
    @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t " +
            "WHERE t.person.id = :personId AND t.transactionType = com.moneytracker.enums.TransactionType.INCOME")
     ...
     */
    @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t " +
            "WHERE t.person.id = :personId AND t.transactionType = com.moneytracker.enums.TransactionType.EXPENSE")
    BigDecimal getTotalExpensesByPersonId(@Param("personId") Long personId);

    /**
     * Get balance for a person (income - expenses)
     */
    @Query("SELECT COALESCE(SUM(CASE WHEN t.transactionType = com.moneytracker.enums.TransactionType.INCOME " +
            "THEN t.amount ELSE -t.amount END), 0) FROM Transaction t WHERE t.person.id = :personId")
    ...
    @Query("SELECT t FROM Transaction t " +
            ...
            "ORDER BY t.transactionDate DESC")
    List<Transaction> getMonthlyTransactions(@Param("personId") Long personId,
                                             @Param("year") int year,
    ...
    @Query("SELECT DISTINCT t.category FROM Transaction t " +
            "WHERE t.person.id = :personId AND t.category IS NOT NULL ORDER BY t.category")
